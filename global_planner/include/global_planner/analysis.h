...
void printPathStats(GlobalPlanner* global_planner,
  ...
  for (int i = 2; i < path.size(); ++i) {
    ...
    if (curr_node.cell_.zIndex() - curr_node.parent_.zIndex() == 0) {
      total_dist_cost +=
          global_planner->getEdgeDist(curr_node.parent_, curr_node.cell_);
    } else {
      total_alt_change_cost +=
          global_planner->getEdgeDist(curr_node.parent_, curr_node.cell_);
    }
    total_risk_cost +=
        global_planner->risk_factor_ * global_planner->getRisk(curr_node);
    total_smooth_cost +=
        global_planner->smooth_factor_ *
        global_planner->getTurnSmoothness(last_node, curr_node);
  }
  ...
  for (int i = 2; i < path.size(); ++i) {
    ...
    double edge_r =
        global_planner->risk_factor_ * global_planner->getRisk(curr_node);
    double edge_s = global_planner->smooth_factor_ *
                    global_planner->getTurnSmoothness(last_node, curr_node);
    ...
    double alt_heuristics = global_planner->altitudeHeuristic(
        curr_node.cell_, goal);  // Lower bound cost due to altitude change
    double smooth_heuristics =
        global_planner->smoothnessHeuristic(curr_node, goal);
    printf("%s: \t%3.2f \t%3.2f \t%3.2f \t%3.2f",
...
template <typename GlobalPlanner>
void printPointStats(GlobalPlanner* global_planner, double x, double y,
                     double z) {
  Cell cell(x, y, z);
  ROS_INFO("\n\nDEBUG INFO FOR %s", cell.asString().c_str());
  ROS_INFO("Rist cost: %2.2f",
           global_planner->risk_factor_ * global_planner->getRisk(cell));
  ROS_INFO("getRisk: %2.2f", global_planner->getRisk(cell));
  ROS_INFO("singleCellRisk: %2.2f", global_planner->getSingleCellRisk(cell));
  ROS_INFO(
      "Neighbors:\n \t %2.2f \t \t \t %2.2f \n %2.2f \t \t %2.2f \n \t %2.2f "
      "\t \t \t %2.2f",
      global_planner->getSingleCellRisk(Cell(x, y + 1, z)),
      global_planner->getSingleCellRisk(Cell(x, y, z + 1)),
      global_planner->getSingleCellRisk(Cell(x - 1, y, z)),
      global_planner->getSingleCellRisk(Cell(x + 1, y, z)),
      global_planner->getSingleCellRisk(Cell(x, y - 1, z)),
      global_planner->getSingleCellRisk(Cell(x, y, z - 1)));

  double heuristics =
      global_planner->getHeuristic(Node(cell, cell), global_planner->goal_pos_);
  ...
  if (node) {
    ...
    if (global_planner->occupied_.find(cell) !=
        global_planner->occupied_.end()) {
      ROS_INFO("Cell in occupied, posterior: %2.2f", post_prob);
    } else {
      ROS_INFO("Cell NOT in occupied, posterior: %2.2f",
               global_planner->expore_penalty_ * post_prob);
    }
  } else {
    ROS_INFO(
        "Cell not in tree, prob: %2.2f",
        global_planner->expore_penalty_ * global_planner->getAltPrior(cell));
  }
}
