                    ...
                    const Cell &goal, double total_cost)
{
    ...
    for (int i = 2; i < path.size(); ++i) {
        ...
        total_smooth_cost += global_planner->smooth_factor_ * global_planner->getTurnSmoothness(last_node,
                             curr_node);
    }
    ...
    printf("%s: \t%3.2f \t%3.2f \t%3.2f \t%3.2f \t|| \n", start.asString().c_str(),
           curr_cost, global_planner->getHeuristic(first_node, goal),
           total_cost, total_cost / global_planner->getHeuristic(first_node, goal));
    ...
    for (int i = 2; i < path.size(); ++i) {
        ...
        double edge_s = global_planner->smooth_factor_ * global_planner->getTurnSmoothness(last_node,
                        curr_node);
        ...
        double alt_heuristics = global_planner->altitudeHeuristic(curr_node.cell_,
                                                                  goal);         // Lower bound cost due to altitude change
        ...
        if (smooth_heuristics > 4 * global_planner->smooth_factor_) {
            ...
            printf("\t|| \t%3.2f \t%3.2f \t%3.2f \t%3.2f \t%3.2f \t%3.2f \n", u_ang, goal_ang, ang_diff,
                   ang_diff2, ang_diff3, num_45_deg_turns);
