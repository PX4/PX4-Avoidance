                     ...
                     double overestimate_factor = 1.0)
{
    ...
    std::cout << std::setw(20) << std::left << node_type <<
              std::setw(10) << std::setprecision(3) << avg_time <<
              std::setw(10) << std::setprecision(3) << overestimate_factor <<
                          ...
                          int max_iterations, Visitor &visitor)
{
    ...
    // printf(" %2.1f µs \t\t %d \t\t %2.2f \t", average_iter_time, num_iter, distance[best_goal_node]);
...
bool find2DPath(GlobalPlanner *global_planner, std::vector<Cell> &path,
                const Cell &s, const Cell &t, const Cell &start_parent, double alt)
{
...
bool reverseSearch(GlobalPlanner *global_planner, const Cell &t)
{
    ...
    while (!pq.empty() && num_iter++ < 100) {
        ...
        for (const Cell &v : u.getNeighbors()) {
            double new_cost = cost[u] + u.distance3D(v) * global_planner->risk_factor_ *
...
template <typename GlobalPlanner>
bool findPathOld(GlobalPlanner *global_planner,
                 std::vector<Cell> &path, const Cell &s,
                 const Cell &t, const Cell &start_parent, bool is_3D)
{
    ...
    while (!pq.empty() && num_iter < global_planner->max_iterations_) {
        ...
        for (const auto &v_cell_dist : neighbors) {
            ...
            if (new_dist < old_dist) {
                ...
                heuristic += global_planner->up_cost_ * std::max(0,
                                                                 t.zIndex() - v.zIndex());       // Minumum cost for increasing altitude
                heuristic += std::max(0, v.zIndex() -
                                      t.zIndex());                // Minumum cost for decreasing altitude
                // if (isNearWall(v)) {
                //   heuristic -= 10;
                // }
    ...
    double average_iter_time = (std::clock() - start_time) / (double)(CLOCKS_PER_SEC / 1000000) /
                               num_iter;
