...
bool findPathOld(GlobalPlanner * global_planner, 
  ...
  while (!pq.empty() && num_iter < global_planner->max_iterations_) {
    ...
    for (const auto & v_cell_dist : neighbors) {
      ...
      if (new_dist < old_dist) {
        ...
        heuristic += global_planner->up_cost_ * std::max(0, t.zIndex() - v.zIndex());       // Minumum cost for increasing altitude
  ...
  // ROS_INFO("Found path with %d iterations, itDistSquared: %.3f", num_iter, num_iter / squared(path.size()));
  printf("%2.2f \t\t %2.2f \t\t %d \t\t %2.2f \t", average_iter_time, global_planner->overestimate_factor_, num_iter, distance[t]);
