...
void GroundDetector::dynamicReconfigureSetGroundParams(avoidance::LocalPlannerNodeConfig & config, uint32_t level) {
...
void GroundDetector::logData(std::string log_name){
  std::ofstream myfile2(("InternalHeightMap_" + log_name).c_str(), std::ofstream::app);
...
void GroundDetector::detectGround(pcl::PointCloud<pcl::PointXYZ>& complete_cloud) {
