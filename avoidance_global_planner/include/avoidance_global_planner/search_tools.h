#ifndef AVOIDANCE_GLOBAL_PLANNER_SEARCH_TOOLS_H_
#define AVOIDANCE_GLOBAL_PLANNER_SEARCH_TOOLS_H_

#include <string>

#include "avoidance_global_planner/bezier.h"
#include "avoidance_global_planner/cell.h"
#include "avoidance_global_planner/node.h"
#include "avoidance_global_planner/visitor.h"

// This file consists of general search tools

namespace avoidance_global_planner {
...
struct SearchInfo {
  ...
  SearchInfo(bool found_path_, int num_iter_, double search_time_)
      : found_path(found_path_),
        num_iter(num_iter_),
        search_time(search_time_) {}
  bool found_path;
  int num_iter;
  double search_time;  // in micro seconds
};

void printSearchInfo(SearchInfo info, std::string node_type = "Node",
                     double overestimate_factor = 1.0) {
  double avg_time = info.search_time / info.num_iter;
  std::cout << std::setw(20) << std::left << node_type << std::setw(10)
            << std::setprecision(3) << avg_time << std::setw(10)
            << std::setprecision(3) << overestimate_factor << std::setw(10)
            << info.num_iter << std::setw(10) << 0.0;
}
...
nav_msgs::Path smoothPath(const nav_msgs::Path& path) {
  ...
  for (int i = 2; i < path.poses.size(); i++) {
    ...
    std::vector<geometry_msgs::Point> smooth_turn =
        threePointBezier(p0, p1, p2);
    for (const auto& point : smooth_turn) {
      geometry_msgs::PoseStamped pose_msg =
          path.poses.front();  // Copy the original header info
...
std::vector<Cell> simplifyPath(GlobalPlanner* avoidance_global_planner,
  ...
  for (int j = 0; j < max_iter; ++j) {
    ...
    for (; i < curr_path.size(); i += 2) {
      ...
      double curr_cost = avoidance_global_planner->getEdgeCost(parent, u) +
                         avoidance_global_planner->getEdgeCost(u, v);
      double new_cost = avoidance_global_planner->getEdgeCost(parent, w);
...
template <typename GlobalPlanner>
SearchInfo findSmoothPath(GlobalPlanner* avoidance_global_planner,
                          std::vector<Cell>& path, const NodePtr& s,
                          const GoalCell& t, int max_iterations = 2000) {
  NullVisitor visitor;
  return findSmoothPath(avoidance_global_planner, path, s, t, max_iterations, visitor);
}
...
SearchInfo findSmoothPath(GlobalPlanner* avoidance_global_planner,
                          std::vector<Cell>& path, const NodePtr& s,
                          const GoalCell& t, int max_iterations,
                          Visitor& visitor) {
  ...
  std::priority_queue<PointerNodeDistancePair,
                      std::vector<PointerNodeDistancePair>, CompareDist>
      pq;
  ...
  while (!pq.empty() && num_iter < max_iterations) {
    ...
    for (const NodePtr& v : u->getNeighbors()) {
      if (!avoidance_global_planner->isLegal(*v)) {
        continue;
      }
      double new_dist = distance[u] + avoidance_global_planner->getEdgeCost(*u, *v);
      ...
      if (new_dist < old_dist) {
        ...
        double overestimated_heuristic =
            new_dist + avoidance_global_planner->getHeuristic(*v, t);
...
template <typename GlobalPlanner>
bool find2DPath(GlobalPlanner* avoidance_global_planner, std::vector<Cell>& path,
                const Cell& s, const Cell& t, const Cell& start_parent,
                double alt) {
  std::vector<Cell> up_path;
  Cell above_s(Cell(s.xIndex(), s.yIndex(), alt));
  Cell above_t(Cell(t.xIndex(), t.yIndex(), alt));
  bool found_up_path =
      findPathOld(avoidance_global_planner, up_path, s, above_s, s, true);
  std::vector<Cell> down_path;
  bool found_down_path =
      findPathOld(avoidance_global_planner, down_path, above_t, t, above_t, true);
  std::vector<Cell> vert_path;
  bool found_vert_path =
      findPathOld(avoidance_global_planner, vert_path, above_s, above_t, above_s, false);
...
template <typename GlobalPlanner>
bool reverseSearch(GlobalPlanner* avoidance_global_planner, const Cell& t) {
  std::map<Cell, double> cost;
  std::priority_queue<CellDistancePair, std::vector<CellDistancePair>,
                      CompareDist>
      pq;
  std::unordered_set<Cell> seen_cells;
  int num_iter = 0;
  cost[t] = avoidance_global_planner->getRisk(t);
  ...
  while (!pq.empty() && num_iter++ < 100) {
    ...
    seen_cells.insert(u);
    avoidance_global_planner->bubble_risk_cache_[u] = cost[u];
    avoidance_global_planner->bubble_cost_ = cost[u];
    avoidance_global_planner->bubble_radius_ =
        std::max(avoidance_global_planner->bubble_radius_, u.distance3D(t));

    for (const Cell& v : u.getNeighbors()) {
      double new_cost = cost[u] +
                        u.distance3D(v) * avoidance_global_planner->risk_factor_ *
                            avoidance_global_planner->getRisk(v);
...
template <typename GlobalPlanner>
bool findPathOld(GlobalPlanner* avoidance_global_planner, std::vector<Cell>& path,
                 const Cell& s, const Cell& t, const Cell& start_parent,
                 bool is_3D) {
  // Initialize containers
  // avoidance_global_planner->seen_.clear();
  ...
  // is found,
  while (!pq.empty() && num_iter < avoidance_global_planner->max_iterations_) {
    CellDistancePair u_cell_dist = pq.top();
    pq.pop();
    Cell u = u_cell_dist.first;
    double d = distance[u];
    ...
    avoidance_global_planner->getOpenNeighbors(u, neighbors, is_3D);
    for (const auto& v_cell_dist : neighbors) {
      Cell v = v_cell_dist.first;
      double edge_cost = v_cell_dist.second;  // Use getEdgeCost
      double risk = avoidance_global_planner->risk_factor_ * avoidance_global_planner->getRisk(v);
      if (risk >
              avoidance_global_planner->risk_factor_ * avoidance_global_planner->max_cell_risk_ &&
          v != t) {
        continue;
      }
      ...
      if (new_dist < old_dist) {
        ...
        // TODO: try Dynamic Weighting instead of a constant overestimate_factor
        double heuristic =
            v.diagDistance2D(t);  // Lower bound for distance on a grid
        heuristic +=
            avoidance_global_planner->up_cost_ *
            std::max(0,
                     t.zIndex() -
                         v.zIndex());  // Minumum cost for increasing altitude
        heuristic += std::max(
            0,
            v.zIndex() - t.zIndex());  // Minumum cost for decreasing altitude
        // if (isNearWall(v)) {
        //   heuristic -= 10;
        // }
        ...
        // }
        double overestimated_heuristic =
            new_dist + avoidance_global_planner->overestimate_factor_ * heuristic;
        pq.push(std::make_pair(v, overestimated_heuristic));
      }
    }
  }
  double average_iter_time = (std::clock() - start_time) /
                             (double)(CLOCKS_PER_SEC / 1000000) / num_iter;
  ...
  printf("%2.2f \t\t %2.2f \t\t %d \t\t %2.2f \t", average_iter_time,
         avoidance_global_planner->overestimate_factor_, num_iter, distance[t]);
