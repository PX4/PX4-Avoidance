#ifndef AVOIDANCE_GLOBAL_PLANNER_NODE
#define AVOIDANCE_GLOBAL_PLANNER_NODE
...
class NodeWithoutSmooth : public Node {
 public:
  ...
  std::size_t hash() const { return std::hash<avoidance_global_planner::Cell>()(cell_); }
...
template <>
struct hash<avoidance_global_planner::Node> {
  std::size_t operator()(const avoidance_global_planner::Node& node) const {
    return node.hash();
  }
};

template <>
struct hash<avoidance_global_planner::NodeWithoutSmooth> {
  std::size_t operator()(const avoidance_global_planner::NodeWithoutSmooth& node) const {
    return node.hash();
  }
};
...
#endif  // AVOIDANCE_GLOBAL_PLANNER_NODE
