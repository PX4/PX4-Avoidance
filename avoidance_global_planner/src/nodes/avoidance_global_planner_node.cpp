#include "avoidance_global_planner_node.h"
...
GlobalPlannerNode::GlobalPlannerNode() {
  ...
  dynamic_reconfigure::Server<
      avoidance_global_planner::GlobalPlannerNodeConfig>::CallbackType f;
...
int main(int argc, char** argv) {
  ...
  avoidance_global_planner::GlobalPlannerNode global_planner_node;
  ...
  if (args.size() > 1) {
    ...
    if (wp_file.is_open()) {
      double x, y, z;
      while (wp_file >> x >> y >> z) {
        global_planner_node.waypoints_.push_back(avoidance_global_planner::Cell(x, y, z));
      }
      wp_file.close();
